...
def main():
    ...
    for p in PREFIX_ORDER:
        if not any(prefix(code) == p for code in books_by_code):
            missing_prefixes.append(p)

    if missing_prefixes:
        raise RuntimeError(f"No books found for prefix(es): {missing_prefixes}")

    import_codes = sorted(
        books_by_code.keys(),
        key=lambda code: (
            int(code.split("-")[1]),
            PREFIX_ORDER.index(prefix(code)),
            int(code.split("-")[2]),
        ),
    )

    expected_count = 469
    if len(import_codes) != expected_count:
        raise RuntimeError(f"Expected {expected_count} books, found {len(import_codes)}")

    now = datetime.now(timezone.utc).isoformat()

    if dry_run:
        print(f"DRY RUN: checking {len(import_codes)} books. Supabase will not be changed.\n")
    else:
        print(f"Importing {len(import_codes)} books into Supabase...\n")
    ...
    for code in import_codes:
        ...
        if not pages:
            raise RuntimeError(f"No page rows found for {code}")

        page_nums = [int(row["page_num"]) for row in pages]
        expected_page_nums = list(range(1, len(pages) + 1))
        if page_nums != expected_page_nums:
            raise RuntimeError(f"Broken page sequence for {code}: {page_nums}")
        ...
        if dry_run:
            map_skills(back)
            if code in (import_codes[0], import_codes[-1]):
                print(f"CHECKED {code:12} pages={len(pages):2} words={total_words:4} title={book_payload['title']}")
            continue
    ...
    expected_pages_total = 7628
    if checked_pages != expected_pages_total:
        raise RuntimeError(f"Expected {expected_pages_total} pages, found {checked_pages}")

    if dry_run:
        print(f"\nDRY RUN PASSED - books={len(import_codes)} pages={checked_pages} skills={len(import_codes)}")
    else:
        print(f"\nRESULT: PASSED - books={len(import_codes)} pages={checked_pages} skills={len(import_codes)}")
