...
(function () {
  ...
  // Only these openers are safe to lowercase after a connective lead; anything
  // else is likely a name ("Mama Nkechi", "Anansi") and keeps its capital.
  var STARTERS = /^(the|a|an|it|they|he|she|there|this|that|we|my|his|her|their|its|some|one|no|not|when|because|after|before|everyone|everybody|nobody|people)$/i;
  function softLower(s) {
    s = tidy(s);
    if (!s) return s;
    var first = s.split(/\s+/)[0].replace(/[^A-Za-z']/g, "");
    if (!first) return s;
    // A word in ALL CAPS or any capitalised word that isn't a plain starter
    // stays as the Captain wrote it.
    if (!STARTERS.test(first)) return s;
  ...
  function joinLead(lead, text) {
    ...
    return /[.!?]\s*$/.test(lead) ? lead + sentence(text) : lead + clause(softLower(text));
  }
  ...
  // Does the answer already carry its own subject and verb?
  function isClause(s) {
    s = tidy(s);
    if (!/^(i|we|they|he|she|it|you|there)\b/i.test(s)) return false;
    return /^\S+\s+\S+/.test(s);
  }
  // Sentence-ending counterparts, used when the answer is a full clause.
  var WHO_LEADS_S = [
    "One name stands out.",
    "{I} will remember one name from this crossing.",
    "The crew of this voyage came down to one.",
  ];
  var NOTICED_LEADS_S = [
    "{I} kept watch.",
    "Not everything is on the chart.",
    "Something caught {my} eye.",
  ];
  var IDEA_LEADS_S = [
    "Here is the true cargo.",
    "Strip away the weather and this is what is left.",
    "This is what the voyage was really about.",
  ];
  var FEELING_LEADS_S = [
    "And when it was done?",
    "{I} closed the cover.",
    "That was the harbour {I_low} reached.",
  ];
  var EXTRA_LEADS_S = [
    "Worth setting down.",
    "One more thing from the crossing.",
  ...
  function write(input) {
    ...
    var p2 = [];
    if (slots.who) p2.push(joinLead(fill(pick(isClause(slots.who) ? WHO_LEADS_S : WHO_LEADS)), slots.who));
    if (slots.noticed) p2.push(joinLead(fill(pick(isClause(slots.noticed) ? NOTICED_LEADS_S : NOTICED_LEADS)), slots.noticed));
    extra.slice(0, 2).forEach(function (e) { p2.push(joinLead(fill(pick(isClause(e) ? EXTRA_LEADS_S : EXTRA_LEADS)), e)); });
    if (slots.word) p2.push(fill(pick(WORD_LEADS)) + "\u201C" + stripEnd(slots.word) + ".\u201D");

    // ---- Paragraph 3: the true cargo, and the harbour ----
    var p3 = [];
    if (slots.idea) p3.push(joinLead(fill(pick(isClause(slots.idea) ? IDEA_LEADS_S : IDEA_LEADS)), slots.idea));
    if (slots.feeling) p3.push(joinLead(fill(pick(isClause(slots.feeling) ? FEELING_LEADS_S : FEELING_LEADS)), slots.feeling));
